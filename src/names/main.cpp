                      ...
                      CValidationState& state, unsigned flags)
{
  ...
  /* Check that no name inputs/outputs are present for a non-Denemecoin tx.
     If that's the case, all is fine.  For a Denemecoin tx instead, there
     should be at least an output (for NAME_NEW, no inputs are expected).  */

  if (!tx.IsDenemecoin ())
    {
      if (nameIn != -1)
        return state.Invalid (error ("%s: non-Denemecoin tx %s has name inputs",
                                     __func__, txid));
      if (nameOut != -1)
        return state.Invalid (error ("%s: non-Denemecoin tx %s at height %u"
                                     " has name outputs",
  ...
  assert (tx.IsDenemecoin ());
  if (nameOut == -1)
    return state.Invalid (error ("%s: Denemecoin tx %s has no name outputs",
                                 __func__, txid));

  /* Reject "greedy names".  */
  const Consensus::Params& params = Params ().GetConsensus ();
  if (tx.vout[nameOut].nValue < params.rules->MinDenemecoinAmount(nHeight))
                      ...
                      CCoinsViewCache& view, CBlockUndo& undo)
{
  ...
  /* This check must be done *after* the historic bug fixing above!  Some
     ...
     transactions *not* marked as Denemecoin tx.  */
  if (!tx.IsDenemecoin ())
    return;
