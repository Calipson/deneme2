...
class BitcoinTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave denemecoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop denemecoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing denemecoind/denemecoin-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: denemecoinds were not stopped and may still be running")
    ...
    def _initialize_chain(self, test_dir, num_nodes, cachedir):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("BITCOIND", "denemecoind"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                args.extend(base_node_args(i))
                if i > 0:
                    args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
                bitcoind_processes[i] = subprocess.Popen(args)
                self.log.debug("initialize_chain: denemecoind started, waiting for RPC to come up")
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "denemecoind"),
                          help="denemecoind binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "denemecoind"),
                          help="denemecoind binary to use for reference nodes (if any)")
