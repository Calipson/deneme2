...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "denemecoin.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    ...
    while True:
        if process.poll() is not None:
            raise Exception('denemecoind exited with status %i during initialization' % process.returncode)
        try:
...
def _start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
    ...
    logger.debug("initialize_chain: denemecoind started, waiting for RPC to come up")
...
def assert_start_raises_init_error(i, dirname, extra_args=None, expected_msg=None):
    with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
        try:
            ...
            assert 'denemecoind exited' in str(e) #node must have shutdown
            ...
            if expected_msg is None:
                assert_msg = "denemecoind should have exited with an error"
            else:
                assert_msg = "denemecoind should have exited with expected error " + expected_msg
            raise AssertionError(assert_msg)

def _start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """Start multiple denemecoinds, return RPC connections to them
