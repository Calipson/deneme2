...
*Setup instructions for a Gitian build of denemecoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the denemecoin
...
- [Building denemecoin](#building-denemecoin)
...
Clone the git repositories for denemecoin-core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/denemecoin/denemecoin-core
git clone https://github.com/denemecoin/gitian.sigs.git
```
...
in the denemecoin-core repository under 'Fetch and create inputs' to install sources which require
...
To build denemecoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the denemecoin-core repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/denemecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/denemecoin/denemecoin-core
...
COMMIT=hardfork-nmc
./bin/gbuild --commit denemecoin=${COMMIT} --url denemecoin=${URL} ../denemecoin-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit denemecoin=${COMMIT} --url denemecoin=${URL} ../denemecoin-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit denemecoin=${COMMIT} --url denemecoin=${URL} ../denemecoin-core/contrib/gitian-descriptors/gitian-osx.yml
```

Building fully offline
-----------------------

**Note: denemecoin does not yet use detatched sigs; this section might not be relevant.**

For building fully offline including attaching signatures to unsigned builds, the detached-sigs repository
and the denemecoin-core git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../denemecoin-core/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/denemecoin/denemecoin-detached-sigs.git

NMCPATH=/some/root/path/denemecoin-core.git
SIGPATH=/some/root/path/denemecoin-detached-sigs.git

./bin/gbuild --url denemecoin=${NMCPATH},signature=${SIGPATH} ../denemecoin-core/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/denemecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/denemecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/denemecoin-osx-build.assert
```
...
[denemecoin/gitian.sigs](https://github.com/denemecoin/gitian.sigs/) repository, or if that's not possible create a pull
